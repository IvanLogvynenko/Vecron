...
std::vector<std::string> listFiles(const std::string &path) {
    if (!std::filesystem::is_directory(path)) { throw std::runtime_error(path + " is a file, not a directory"); }

    return std::ranges::to<std::vector<std::string>>(
        std::filesystem::directory_iterator(path) |
        std::views::filter([](const auto &entry) { return !entry.is_directory(); }) |
        std::views::transform([](const auto &entry) { return entry.path().filename().string(); }));
}
    ...
    while (!unvisited_directories.empty()) {
        ...
        for (const auto &entry : std::filesystem::directory_iterator(directory)) {
            if (entry.is_directory())
                ...
                result.push_back(entry.path().string().substr(path.length())); // path, relative to template directory
        }
    }
