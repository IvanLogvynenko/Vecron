...
LocalConfiguration::LocalConfiguration(std::string configPath) : _configPath(std::move(configPath)) {
    ...
    if (std::filesystem::exists(_configPath + "main.json")) {
        ...
        _data = nlohmann::json::parse(mainFile);
...
const std::optional<std::string> &LocalConfiguration::getBuildCMD() noexcept {
    if (_data.contains("build")) {
        this->_build = this->_data["build"];
    } else {
        this->_build = std::nullopt;
    }
    return this->_build;
